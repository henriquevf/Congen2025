...
```
Let's reorganize the file: 
```
cut -d' ' -f1-7 PCA_Elephants.eigenvec > PCA_Elephants.txt 
cat /anvil/projects/x-bio240351/shared_data/outputs/header.txt PCA_Elephants.txt > PCA_Elephants_header.txt
expand -t 1 sample_info.txt > sample_info_single.txt
paste sample_info_single.txt PCA_Elephants_header.txt > PCA_Elephants_final.txt
```
