...
This prints a map of our study area, the edges, and the sample points.

Lets look at the output here in an example:

![image](https://github.com/user-attachments/assets/1527dda0-de11-4f1f-a8d1-39e404cc7816)

Here we can see the sample points and the dense grid. FEEMS covers the map with a dense grid to estimate population structure and gene flow across a landscape. This has no genetic data yet.

- Each deme exchanges migrants only with its neighbors.
- Using a stepping-stone model, we can estimate expected genetic dissimilarities based on the sample location and migration rates.

### Let's just explore what the best lambda is, which determines "smoothness"

the Lambda value optimizes the tradeoff between our gene flow model being very specific to our dataset vs. capturing more general patterns. This is called the Bias-Variance tradeoff.

- Specifically, it is the relationship between model complexity (the accuracy of its predictions) and how well it can make predictions on unseen data.
...
![image](https://github.com/user-attachments/assets/91cb4bc8-0bfe-4b05-923a-0f9b44dfa703)


This provides our best lambda model of 2.

### Run final analysis 

Now we can run our final analysis for feems.
...
![image](https://github.com/user-attachments/assets/601d9d65-0656-496b-b034-9acf6a59104f)

Now we can take a look at the barriers!

- Red colors are where gene flow is less than expected based on isolation-by-distance
- 
- Blue colors are where gene flow is more than expected based on isolation-by-distance
