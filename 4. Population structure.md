...
OUT=/anvil/scratch/x-YOURNAME/shared_data/04_populationstructure/LoxAfr4_elephant_no1stdegree_variable_sites_nomultiallelics_noindels_10dp_3het_chr1_ForestvsSavannah
...
```
less /anvil/projects/x-bio240351/shared_data/vcf/LoxAfr4_elephant_no1stdegree_variable_sites_nomultiallelics_noindels_10dp_3het_chr1_ForestvsSavannah.weir.fst
```
And remove the -NAN
```
sed '/n/d' LoxAfr4_elephant_no1stdegree_variable_sites_nomultiallelics_noindels_10dp_3het_chr1_ForestvsSavannah.weir.fst >  LoxAfr4_elephant_no1stdegree_variable_sites_nomultiallelics_noindels_10dp_3het_chr1_ForestvsSavannah_noNAN.weir.fst
```
Now we can calculate the average Fst value between Forest and Savannah Elephants 
```
awk '{ sum += $3 } END { print "Average Fst between Savannah and Forest Elephants:", sum/NR }' LoxAfr4_elephant_no1stdegree_variable_sites_nomultiallelics_noindels_10dp_3het_chr1_ForestvsSavannah_noNAN.weir.fst > averageFst_Savannah_Forest_Elephants.txt
```
...
library (ggplot2)
#read dataset
dat <- read.table("LoxAfr4_elephant_no1stdegree_variable_sites_nomultiallelics_noindels_10dp_3het_chr1_ForestvsSavannah_noNAN.weir.fst", sep="\t",header=T)

#Look at dataset
head(dat)

##subset the dataset
dat1 <- dat[dat[,2] > 3000155,]
dat_plot <- dat1[dat1[,2] < 3100155,]

#plot data
x <- ggplot(dat, aes(x=POS, y=WEIR_AND_COCKERHAM_FST)) + theme_classic() + geom_line(size=1.1) 
ggsave("Fst_Savannah_Forest_Elephant.tiff", width=10,height=6)
