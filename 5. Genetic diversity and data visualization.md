...
#SBATCH -o /anvil/scratch/YOUR_USERNAME/logs/avg_het.o%j      # Name of stdout output file
#SBATCH -e /anvil/scratch/YOUR_USERNAME/logs/avg_het.e%j      # Name of stderr error file
...
You can plot the results with R in your local computer or in the Anvil server.
If you have experience using R and want to use it in your own computer, download the file `elephants_heterozygosity.tsv` to your local computer and adjust the paths in the script below accordingly.
If you choose to use R in Anvil server (a simpler option if you lack extensive experience), first load the R module by entering `module load r/4.1.0` in the terminal. Then, start an R session by typing `R`.
Make sure you understand each command in the R script with the help of the explanations provided in the commented lines (#).
...
#SBATCH --mem=512M          # Memory allocation
#SBATCH --time=0:30:00    # Total run time limit (hh:mm:ss)
#SBATCH -o /anvil/scratch/YOUR_USERNAME/logs/genome_wide_het.o%j      # Name of stdout output file
#SBATCH -e /anvil/scratch/YOUR_USERNAME/logs/genome_wide_het.e%j      # Name of stderr error file
#SBATCH -p wholenode  # Queue (partition) name
...
>[!NOTE]
>This script run ANGSD for a single individual defined in the variable `$SAMPLE`. Adjust the script to run it for a second sample named `CH0893`.
...
#SBATCH --time=0:30:00    # Total run time limit (hh:mm:ss)
#SBATCH -o /anvil/scratch/YOUR_USERNAME/logs/prep_genome_wide_het.o%j      # Name of stdout output file
#SBATCH -e /anvil/scratch/YOUR_USERNAME/logs/prep_genome_wide_het.e%j      # Name of stderr error file
#SBATCH -p wholenode  # Queue (partition) name

# Set the input and output directories
input_directory="/anvil/scratch/YOUR_USERNAME/05_genetic_diversity"
output_directory="/anvil/scratch/YOUR_USERNAME/05_genetic_diversity"
