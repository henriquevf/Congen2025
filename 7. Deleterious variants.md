...
low_vcf_file <- "low_impact_variants.vcf"
moderate_vcf_file <- "moderate_impact_variants.vcf"
high_vcf_file <- "high_impact_variants.vcf"
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Plot SFS
sfs_plot <- ggplot(SFS, aes(x = totalt, y = Rel, fill = Type)) +
...
# Plot SFS with x-axis limited to a maximum of 100
sfs_plot <- ggplot(SFS, aes(x = totalt, y = Rel, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(x = "Alt alleles", y = "Fraction of sites") +
  xlim(0, 100)  # Limit the x-axis from 0 to 100

# Save SFS plot as PNG
ggsave("SFS_plot.png", sfs_plot, width = 8, height = 6, dpi = 300)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Building SFS with relative sizes
SFS_rel <- SFS %>% 
  group_by(Type) %>% 
  mutate(Rel = count / sum(count))  # Calculate the relative proportion

# Plot the SFS with relative sizes
sfs_rel_plot <- ggplot(SFS_rel, aes(x = totalt, y = Rel, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(x = "Alt alleles", y = "Relative number of sites", 
