...
### Check quality of bam and reads 
We can take a look at the read quality within our bam file to better understand the alignment process. 

What may lead to a poor mapping quality? 
> low complexity region, the read maps well to many other regions (retrotransposon insertion), read itself is poor quality (ancient DNA), poor reference genome 

We can use flagstat to check proportion of mapped reads: 

```
touch 02_bamquality.sh
```
...
#SBATCH --time=1:30:00
#SBATCH --job-name alignment
#SBATCH -o /anvil/scratch/YOUR_USERNAME/log/02_bamquality.o%j
#SBATCH -e /anvil/scratch/YOUR_USERNAME/log/02_bamquality.e%j

module load samtools

samtools flagstat /anvil/projects/x-bio240351/shared_data/bams/NA5206.loxAfr4_NC000934_subsampled.bam
```
Now look at the output file: 
```
cd /anvil/scratch/YOUR_USERNAME/log
less 02_bamquality.o%
```
What do properly paired, mapped, singletons mean? 

And we can use the Integrative Genome Viewer (IGV) to visualize our reads mapped to the reference genome. 
